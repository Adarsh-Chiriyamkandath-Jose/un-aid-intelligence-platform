...
npm install
Environment Configuration Create .env file in project root:
...
OpenAI: Create account at platform.openai.com → API Keys
Database Setup
...
Data Ingestion: Automatic CSV parsing and validation on startup
Database Population: Chunked inserts with foreign key relationships
Real-time Analytics: Dynamic aggregation queries for dashboard metrics
ML Pipeline: Feature engineering → Model training → Prediction → SHAP analysis
API Documentation
Core Endpoints
Dashboard Analytics
...
Prophet: Time series decomposition with trend, seasonality, and holiday effects
XGBoost: Gradient boosting with feature engineering (GDP, political stability, disasters)
Hybrid Ensemble: Weighted combination optimizing for accuracy and interpretability
Feature Engineering
...
Source: Combined dataset merging official UN development assistance records with World Bank World Development Indicators (WDI) Volume: 516MB, 478,371 records, $952.3M total aid Coverage: 14 recipient countries, 146 donor organizations, 53 sectors with economic indicators
