...
async def load_real_aid_data(db: Session):
    """Load real UN aid data from the CSV file with memory efficiency"""
    ...
    try:
        ...
        if existing_count < 1000:
            ...
            logger.info(f"Found {existing_count} existing records - assuming real data already loaded")
            return

        # Load real UN aid data efficiently using pandas chunks
        csv_path = './merged_data.csv'  # Use the authentic merged_data.csv file
        ...
        for chunk_num, chunk in enumerate(pd.read_csv(csv_path, chunksize=chunk_size)):
            ...
            logger.info(f"Processing chunk {chunk_num + 1}, records processed: {total_processed}")
            ...
            for _, row in chunk.iterrows():
                ...
                try:
                    # Extract and clean data using proper pandas operations
                    ...
                    country_name = str(row.get('RecipientName', '')).strip().title()  # Proper case
                    ...
                    # Amount - prefer disbursement, fallback to commitment, include ALL financial flows
                    ...
                    # Handle NaN values properly and include negative amounts (debt relief, returns)
                    disbursement = 0.0 if pd.isna(disbursement) or disbursement is None else float(disbursement)
                    commitment = 0.0 if pd.isna(commitment) or commitment is None else float(commitment)
                    amount = disbursement if disbursement != 0 else commitment
                    
                    # Detailed validation and logging for skipped records
                    skip_reason = None
                    if not year or year < 2000 or year > 2030:
                        skip_reason = f"invalid_year_{year}"
                    elif not country_code or country_code == '' or country_code == 'nan':
                        skip_reason = "missing_country_code"
                    elif not country_name or country_name == '' or country_name == 'nan':
                        skip_reason = "missing_country_name"
                    elif not donor_code or donor_code == '' or donor_code == 'nan':
                        skip_reason = "missing_donor_code"
                    elif not donor_name or donor_name == '' or donor_name == 'nan':
                        skip_reason = "missing_donor_name"
                    elif not sector_code or sector_code == '' or sector_code == 'nan':
                        # Use default sector for missing sector codes
                        sector_code = '99998'
                        sector_name = 'Unspecified Sector'
                    elif not sector_name or sector_name == '' or sector_name == 'nan':
                        # Use default name for missing sector names
                        sector_name = f'Sector {sector_code}'
                    # Remove zero amount filtering - keep ALL financial records including administrative costs
                    # elif amount == 0:
                    #     skip_reason = f"zero_amount"
                    ...
                    if country_code not in processed_countries:
                        countries_to_add.append({
                            ...
                            'latitude': 0.0,
                            'longitude': 0.0,
                            'population': 0,  # Will be populated from World Bank data later
                            'gdp_per_capita': 0.0
                        })
                    ...
                    skipped_reasons[f"processing_error_{error_type}"] = skipped_reasons.get(f"processing_error_{error_type}", 0) + 1
                    logger.warning(f"Error processing row {total_processed + total_skipped}: {e}")
                    continue
            
            # Use chunked bulk_save_objects to avoid parameter limits
            try:
                # Insert countries in small batches
                if countries_to_add:
                ...
                # Insert donors in small batches  
                if donors_to_add:
                ...
                # Insert sectors in small batches
                if sectors_to_add:
                ...
                # Insert aid records in small batches
                ...
                # Commit after each chunk to avoid memory overload
                db.commit()
                logger.info(f"Committed chunk {chunk_num + 1} - Total processed: {total_processed}")
        ...
        logger.info(f"DATA LOADING AUDIT REPORT:")
        logger.info(f"✓ Successfully processed: {total_processed:,} records")
        logger.info(f"✗ Skipped records: {total_skipped:,}")
        logger.info(f"📊 Processing rate: {(total_processed/(total_processed+total_skipped)*100):.1f}%")
        ...
        logger.info(f"FINAL TOTALS: {len(processed_donors)} donors, {len(processed_countries)} countries, {len(processed_sectors)} sectors")
        logger.info(f"COVERAGE ANALYSIS: {len(processed_donors)}/146 donors ({len(processed_donors)/146*100:.1f}%), {total_processed}/478371 records ({total_processed/478371*100:.1f}%)")
        
        # Log any remaining missing donors for final audit
        if len(processed_donors) < 146:
            logger.info(f"MISSING DONORS: {146 - len(processed_donors)} donors still not captured")
...
def get_donor_country(donor_name: str) -> str:
    ...
    name = donor_name.lower()
    if 'netherlands' in name: return 'Netherlands'
    elif 'germany' in name: return 'Germany' 
    elif any(c in name for c in ['united states', 'usaid', 'usa']): return 'United States'
    elif 'canada' in name: return 'Canada'
    elif any(c in name for c in ['uk', 'britain', 'dfid']): return 'United Kingdom'
    elif 'france' in name: return 'France'
    elif 'japan' in name: return 'Japan'
    elif 'norway' in name: return 'Norway'
    elif 'sweden' in name: return 'Sweden'
